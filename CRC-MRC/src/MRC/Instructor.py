...
class Instructor:
    def __init__(self, opt):
        ...
        if opt.textname == "text":
            self.model = opt.model(num_class = self.num_class, device = opt.device, textname=opt.textname)
        else:
            self.model = opt.model(num_class = self.num_class, device = opt.device, textname=opt.textname)
        ...
        for epoch in range(total_epochs):
            for step, (input_ids, token_type_ids, attention_mask,labels) in enumerate(train_loader):
                ...
                if strategy == "start":
                    ...
                    if (step + 1) % save_step == 0:
                        count += 1
                        train_acc = metrics.accuracy_score(total_label_lt, total_predict_lt)
    ...
    def run_test(self, criterion, optimizer, device, test_loader, strategy, load_model_path=None):
        if load_model_path:
            print(f'load model from {load_model_path}')
            self.model.load_state_dict(torch.load(load_model_path))
            # acc,acc_end,pearson,pearson_end,devloss = self.test(self.model, criterion, test_loader, device, strategy)
            ...
            with torch.no_grad():
                ...
                for step, (input_ids, token_type_ids, attention_mask, labels) in tqdm(enumerate(test_loader),desc='Test Itreation:'):
                    ...
                    if 'RoBERTa' in self.opt.model_name: 
                        predict_start,predict_end = self.model(input_ids, attention_mask)
                    else:
                        predict_start,predict_end = self.model(input_ids, token_type_ids, attention_mask)
                    ...
                    # total_predict_lt_end.extend(predict_end.argmax(dim=1).tolist())
                    # total_predict_lt_all_end.extend(torch.exp(predict_end).tolist())
                    self.opt.logger.info("total_predict_lt:{}".format(predict_start.argmax(dim=1).tolist()))
                    self.opt.logger.info("total_label_lt:{}".format(labels.argmax(dim=1).tolist()))
                    self.opt.logger.info("total_predict_lt_all:{}".format(torch.exp(predict_start).tolist()))
            ...
            stat, p1 = wilcoxon(total_label_lt, total_predict_lt)
            stat, p2 = wilcoxon(total_label_lt_all, total_predict_lt_all)
