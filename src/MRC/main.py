...
from build_csvdata import concat_A_C_ChatGLM_SemEval, concat_A_TC_ChatGLM, concat_A_C_ChatGPT_SemEval, concat_A_C_baichuan_SemEval, concat_A_TC_baichuan, concat_A_C_ChatGPT_ISEAR, concat_A_TC_ChatGPT, process_ISEAR_dataset
...
def main():
    ...
    parser.add_argument('--model_name', default='SinaNews_BERT_MRC_A+C', type=str)# SinaNews_BERT_MRC_A+C SinaNews_RoBERTa_MRC_A+C SinaNews_ERNIE_MRC_A+C
    parser.add_argument('--dataset_name', default='SinaNews_NEW', type=str,choices=['SemEval','ISEAR_NEW','SinaNews_NEW'], help='数据集名称')
    parser.add_argument('--DataSet_Name', default = "SinaNews_Real_text", type=str)#SemEval_MRC_ChatGLM SemEval_MRC_all SinaNews_ChatGLM SinaNews_baichuan SemEval_MRC_ChatGPT SinaNews_ChatGPT
    ...
    parser.add_argument('--device', default='cuda:6', type=str, help='e.g. cuda:0')
    ...
    parser.add_argument('--textname', default = 'Real_text', type=str)#'All_text_cluster' 'All_text_zero'
    # parser.add_argument('--questionname', default = 'text_question', type=str)#'text_question' 'All_question'
    ...
    parser.add_argument('--load_model_path', default="04_All/save_model/ISEAR_NEW/ISEAR_BERT_MRC_A+C_ACC@1=0.7413_AP=0.7335_START.pt", type=str, help='搭配run_test使用,直接加载模型跑测试')#04_All/save_model/SinaNews_NEW/SinaNews_BERT_MRC_A+C_ACC@1=0.7492_AP=0.8314_START.pt   #04_All/save_model/SemEval/SemEval_BERT_MRC_A+C_ACC@1=0.58_AP=0.6479_END.pt  #04_All/save_model/ISEAR_NEW/ISEAR_BERT_MRC_A+C_ACC@1=0.7413_AP=0.7335_START.pt
    ...
    parser.add_argument('--run_saved_model',default=True ,type=bool,help='是否调用已保存的模型')
    parser.add_argument('--pearson_save_target', default=0.58, type=float, help='达到一定准确率才保存模型')
    parser.add_argument('--accuracy_save_target', default=0.63, type=float, help='达到一定准确率才保存模型')
    ...
    if not os.path.exists('04_All/newlogs'):
        os.mkdir('04_All/newlogs')
    opt.logger = logging.getLogger()
    opt.logger.setLevel(logging.INFO)
    opt.logger.addHandler(logging.StreamHandler(sys.stdout))
    opt.logger.addHandler(logging.FileHandler(os.path.join('04_All/newlogs', opt.log_name)))
    # 打印LOG
    ...
    if opt.run_test:
        ins.run_test(criterion,optimizer,ins.opt.device,ins.dev_loader,ins.opt.strategy,ins.opt.load_model_path)
    else:
        torch.cuda.empty_cache()
